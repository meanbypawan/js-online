...
export const signin = async (request, response, next) => {
    try {
        ...
        if (user) {
            ...
            if (status){
                request.session.currentUserId = user.email;
                let payload = {subject: user.email};
                let token = jwt.sign(payload,'fdfxvcvnreorevvvcrerer',{expiresIn: 60});      
                return response.status(200).json({ message: "Sign in success", token: token,status: true,email: request.session.currentUserId });
            }
