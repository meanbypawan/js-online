...
function Shop() {
  ...
  const navigateToDescription = (product)=>{
      ...
      {!error && productList.map((product,index)=> <div key={index} className="col-lg-3 col-md-4 col-sm-6 pb-1">
        <div className="product-item bg-light mb-4">
          ...
          <div className="text-center py-4">
            ...
            <p onClick={()=>navigateToDescription(product)}style={{cursor: 'pointer',color: 'black', fontStyle: 'bold', textDecoration: 'none'}}>View Description</p>
          </div>
        </div>
      </div>)} 
